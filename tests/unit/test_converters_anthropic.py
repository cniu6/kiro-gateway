...
class TestAnthropicToKiro:
    ...
    def test_includes_system_prompt_as_array(self):
        """
        What it does: Verifies that system prompt as array (Claude Code format) is handled.
        Purpose: Ensure Anthropic's system field with cache_control blocks is supported.
        """
        print("Setup: Request with system prompt as array (Claude Code format)...")
        request = AnthropicMessagesRequest(
            model="claude-sonnet-4-5",
            messages=[AnthropicMessage(role="user", content="Hello!")],
            max_tokens=1024,
            system=[
                ...
                {"type": "text", "text": "You help with coding tasks.", "cache_control": {"type": "ephemeral"}}
            ]
        )
        ...
        current_content = result["conversationState"]["currentMessage"]["userInputMessage"]["content"]
        print(f"Current content: {current_content}")
        assert "You are Claude Code." in current_content
        assert "You help with coding tasks." in current_content
